<template>
    ...
    >
      ...
      <tbody class="dark:bg-dark-1 bg-white">
        <tr
          ...
          v-for="(drivers, index) in filteredData"
          :key="drivers.name"
        >
          <template v-if="inRange(index)">
            ...
            <td class="px-4 py-4 rounded-r-lg">
              <a
                target="_blank"
                :href="drivers.wikipedia_link"
                class="tracking-wider text-primary"
                >{{ drivers.wikipedia_link }}</a
              >
            </td>
          </template>
        </tr>
  ...
  <div class="mt-2 flex flex-row justify-between items-center">
    <span class="dark:text-white bg-primary/20 px-2 rounded border border-primary text-sm">
      <template v-if="totalDrivers === '0'">NO DRIVERS FOUND</template>
      <template v-else-if="totalDrivers === '1'"> {{ totalDrivers }} DRIVER </template>
      <template v-else> {{ totalDrivers }} DRIVERS </template>
    </span>
...
const totalDrivers = computed(() => filteredData.value.length.toString())

const pages = computed(() => Math.ceil(filteredData.value.length / perPages.value))

const filteredData = computed(() => {
  if (!searchKey.value) return store.driversData
  return store.driversData.filter((driver) =>
    driver.name.toLowerCase().includes(searchKey.value.toLowerCase())
  )
})
