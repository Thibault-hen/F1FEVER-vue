<template>
    ...
    >
      ...
      <thead class="dark:bg-dark-1 bg-white-2">
        <tr class="text-left tracking-wide text-sm md:text-base">
          ...
          <th class="px-2 py-4 rounded-l-lg"><span class="uppercase text-sm">Location</span></th>
  ...
  <div class="mt-2 flex flex-row justify-between items-center">
    <span class="dark:text-white bg-primary/20 px-2 rounded border border-primary text-sm">
      <template v-if="totalCircuits === '0'">NO CIRCUITS FOUND</template>
      <template v-else-if="totalCircuits === '1'"> {{ totalCircuits }} CIRCUITS </template>
      <template v-else> {{ totalCircuits }} CIRCUITS </template>
    </span>
...
const totalCircuits = computed(() => filteredData.value.length.toString())

const pages = computed(() => Math.ceil(filteredData.value.length / perPages.value))

const filteredData = computed(() => {
  if (!searchKey.value) return store.circuitsData
  return store.circuitsData.filter((circuits) =>
    circuits.name.toLowerCase().includes(searchKey.value.toLowerCase())
  )
})
