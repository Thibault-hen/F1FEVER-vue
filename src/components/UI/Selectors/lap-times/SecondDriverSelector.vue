<template>
  <div class="relative flex">
    <Listbox v-model="selectedSecondDriver" as="div" class="w-full" v-slot="{ open }">
      <ListboxButton
      ...
      >
        <div class="flex flex-row">
          <NationalityFlag
            v-if="selectedSecondDriver.nationality"
            class="mr-2 flex items-center"
            :nationality="selectedSecondDriver.nationality"
          />
          <span class="truncate">{{ selectedSecondDriver.name ?? 'Loading drivers...' }}</span>
        </div>
...
const getDrivers = async (season, grandPrix) => {
  ...
  selectedSecondDriver.value = drivers.value[0]
}

const emitDriver = () => {
  emit('update:modelValue', selectedSecondDriver.value)
}
